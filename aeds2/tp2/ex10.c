...
int getMaior(Restaurante** restaurantes, int n) {

    int maior = restaurantes[0]->capacidade;

    for (int i = 1; i < n; i++) {

        if (maior < restaurantes[i]->capacidade) {

            maior = restaurantes[i]->capacidade;
...
void countingsort(Restaurante** restaurantes, int n) {

    if (n <= 0) return;

    int tamCount = getMaior(restaurantes, n) + 1;
    
    int* count = (int*)malloc(tamCount * sizeof(int));
    Restaurante** ordenado = (Restaurante**)malloc(n * sizeof(Restaurante*));

    for (int i = 0; i < tamCount; i++) count[i] = 0;

    for (int i = 0; i < n; i++) count[restaurantes[i]->capacidade]++;

    for (int i = 1; i < tamCount; i++) count[i] += count[i - 1];

    for (int i = n - 1; i >= 0; i--) {

        ordenado[count[restaurantes[i]->capacidade] - 1] = restaurantes[i];
        count[restaurantes[i]->capacidade]--;

    }

    for (int i = 0; i < n; i++) {

        restaurantes[i] = ordenado[i];
...
void Exercicio_08(Colecao_Restaurantes* colecao_completa) {
 ...
 inicio = clock();
    ...
    countingsort(vetor_ID, tamanho_Vetor);
    ...
    // LOG
    FILE* log = fopen("850602_<countingsort>.txt", "w");
    if (log != NULL) {

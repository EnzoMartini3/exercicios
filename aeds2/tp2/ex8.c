...
void quicksortRec(Restaurante** restaurantes, int esquerda, int direita, long* qntd_comparacoes) {
    int i = esquerda, j = direita;
    

    Restaurante* pivo = restaurantes[(esquerda + direita) >> 1];

    while (i <= j) {

        (*qntd_comparacoes)++;

        while (restaurantes[i]->avaliacao < pivo->avaliacao || 
              (restaurantes[i]->avaliacao == pivo->avaliacao && 
               strcmp(restaurantes[i]->nome, pivo->nome) < 0)) {
            i++;
            (*qntd_comparacoes)++;
    ...
    (*qntd_comparacoes)++;
        while (restaurantes[j]->avaliacao > pivo->avaliacao || 
              (restaurantes[j]->avaliacao == pivo->avaliacao && 
               strcmp(restaurantes[j]->nome, pivo->nome) > 0)) {
            j--;
            (*qntd_comparacoes)++;
        }
    ...
    if (esquerda < j) quicksortRec(restaurantes, esquerda, j, qntd_comparacoes);
    if (i < direita) quicksortRec(restaurantes, i, direita, qntd_comparacoes);
}

void quicksort(Restaurante** restaurantes, int tamanho, long* qntd_comparacoes) {

    if (tamanho > 0) {

        quicksortRec(restaurantes, 0, tamanho - 1, qntd_comparacoes);
    }

}


void Exercicio_08(Colecao_Restaurantes* colecao_completa) {
    ...
    while (scanf("%d", &valor_ID) == 1 && valor_ID != -1) {
    ...
    quicksort(vetor_ID, tamanho_Vetor,&qntd_comparacoes);
    ...
    FILE* log = fopen("850602_<quicksort>.txt", "w");
    if (log != NULL) {

        fprintf(log, "850602\t%ld\t%lf\n", qntd_comparacoes, tempo);
        fclose(log);

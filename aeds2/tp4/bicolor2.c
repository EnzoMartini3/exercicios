...
// Estrutura do Nó modificada para incluir a cor
typedef struct No {
    Restaurante* elemento;
    struct No* esq;
    struct No* dir;
    bool cor; // true para RED, false para BLACK
} No;
...
No* novo_no(Restaurante* r) {
    ...
    temp->esq = NULL;
    temp->dir = NULL;
    temp->cor = RED; // Todo nó inserido nasce VERMELHO
    return temp;
}

// Métodos de Rotação da Árvore Bicolor
...
void inverter_cores(No* no) {
    no->cor = RED;
    if (no->esq != NULL) no->esq->cor = BLACK;
    if (no->dir != NULL) no->dir->cor = BLACK;
}

// Inserção balanceada da Árvore Bicolor
No* inserir_bicolor(No* no, Restaurante* r) {
    if (no == NULL) {
        return novo_no(r);
    }

    if (no->esq != NULL && no->esq->cor == RED && no->dir != NULL && no->dir->cor == RED) {
        inverter_cores(no);
    }

    int comp = strcmp(r->nome, no->elemento->nome);
    if (comp < 0) {
        no->esq = inserir_bicolor(no->esq, r);
    } else if (comp > 0) {
        no->dir = inserir_bicolor(no->dir, r);
    }

    if (no->dir != NULL && no->dir->cor == RED && (no->esq == NULL || no->esq->cor == BLACK)) {
        no = rotacionar_esq(no);
    }
    if (no->esq != NULL && no->esq->cor == RED && no->esq->esq != NULL && no->esq->esq->cor == RED) {
...
No* inserir_arvore(No* raiz, Restaurante* r) {
    raiz = inserir_bicolor(raiz, r);
    if (raiz != NULL) {
        raiz->cor = BLACK;
    }
    return raiz;
...
Restaurante* parse_restaurante(char* str) {
    ...
    sscanf(str, "%d,%99[^,],%99[^,],%d,%lf,%199[^,],%9[^,],%9[^-]-%9[^,],%14[^,],%9s", 
        &id, nome, cidade, &capacity, &avaliacao, tipos_cozinha, 
    ...
    restaurante->capacidade = capacity;
...
int main() {
    ...
    FILE* log = fopen("matrícula_arvore_bicolor.txt", "w");
    if (log != NULL) {

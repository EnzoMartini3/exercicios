...
NoTrie* novo_no() {
    NoTrie* temp = (NoTrie*)malloc(sizeof(NoTrie));
    temp->elemento = NULL;
    temp->folha = false;
...
void inserir_trie(NoTrie* raiz, Restaurante* r) {
    NoTrie* atual = raiz;
    for (int i = 0; r->nome[i] != '\0'; i++) {
        unsigned char ch = r->nome[i];
        if (atual->filhos[ch] == NULL) {
            atual->filhos[ch] = novo_no();
        }
        atual = atual->filhos[ch];
    }
    atual->folha = true;
    atual->elemento = r;
}

bool pesquisar_trie(NoTrie* raiz, char* nome_busca, long long* comparacoes) {
    NoTrie* atual = raiz;
    for (int i = 0; nome_busca[i] != '\0'; i++) {
        unsigned char ch = nome_busca[i];
        printf("%c ", ch);
        (*comparacoes)++;
        if (atual->filhos[ch] == NULL) {
            return false;
...
void limpar_trie(NoTrie* no) {
    if (no == NULL) return;
    for (int i = 0; i < TAMANHO_ALFABETO; i++) {
        if (no->filhos[i] != NULL) {
            limpar_trie(no->filhos[i]);
        }
    }
    free(no);
}
...
int main() {
    ...
    while (fgets(entrada, sizeof(entrada), stdin)) {
        ...
        if (!pesquisar_trie(raiz, entrada, &qntd_comparacoes)) {
            printf("NAO\n");
        }
    }
    ...
    FILE* log = fopen("matrícula_arvore_trie_lista", "w");
    if (log != NULL) {

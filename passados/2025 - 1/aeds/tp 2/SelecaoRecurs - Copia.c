...
int encontrar_menor(Show array[], int i, int n, int min) {
    if (i >= n) return min;
    comparacoes++;
    if (strcmp(array[i]._title, array[min]._title) < 0)
        min = i;
    return encontrar_menor(array, i + 1, n, min);
}

void selecao_recursiva(Show array[], int index, int n) {
    if (index >= n - 1) return;
    int min = encontrar_menor(array, index + 1, n, index);
    if (min != index) {
        Show temp = array[index];
        array[index] = array[min];
        array[min] = temp;
    }
    selecao_recursiva(array, index + 1, n);
}
...
int main() {
    ...
    while (1) {
        ...
        if (buscar_por_show_id("/tmp/disneyplus.csv", entrada, &shows[count])) {
            count++;
        }
    }

    clock_t inicio = clock();
    selecao_recursiva(shows, 0, count);
    clock_t fim = clock();

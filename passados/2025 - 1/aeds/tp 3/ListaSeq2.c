...
void mostrar_lista(ListaSeq1* l) {
    for (Celula* i = l->prim; i != NULL; i = i->prox) {
        imprimir_show(&i->show);
    }
}
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (sscanf(linha, "%s %s %s", comando, arg1, arg2) >= 1) {
            if (strcmp(comando, "II") == 0) {
                if (buscar_por_show_id(path, arg1, &temp)) inserir_inicio(&lista, temp);
            } else if (strcmp(comando, "IF") == 0) {
                if (buscar_por_show_id(path, arg1, &temp)) inserir_fim(&lista, temp);
            } else if (strcmp(comando, "I*") == 0) {
                pos = atoi(arg1);
                if (buscar_por_show_id(path, arg2, &temp)) inserir_pos(&lista, temp, pos);
            } else if (strcmp(comando, "RI") == 0) {
                Show r = remover_inicio(&lista);
                printf("(R) %s\n", r._title);
            } else if (strcmp(comando, "RF") == 0) {
                Show r = remover_fim(&lista);
                printf("(R) %s\n", r._title);
            } else if (strcmp(comando, "R*") == 0) {
                pos = atoi(arg1);
                Show r = remover_pos(&lista, pos);
                printf("(R) %s\n", r._title);
            }
        }
    }
